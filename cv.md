# Pylko Nataliia

mail: [pylko.nata@gmail.com](mailto:pylko.nata@gmail.com)  
telegram: [NataPylko](https://t.me/NataPylko)  
![Image][1]
_My goal is to change the scope of my work, acquire new skills necessary to be a qualified front-end developer_

---

## Skills:

- HTML5 (basic/elementary)
- CSS, SASS (basic/elementary)
- Figma (working with layers)
- Bootstrap 5 (saw it)
- JS (basics)

---

## Projects:

1. [My CV on netlify](https://pylkonatadreamresume.netlify.app/)
2. [My Markdown CV](https://pylkonata.github.io/Archive_rsschool-cv_q1/)
3. [Wildlife page](https://rolling-scopes-school.github.io/pylkonata-JSFE2021Q1/wildlife/)

---

## Work Experience:

### Apprentice RS School

_February **2021** - March **2021** | Ukraine_

- Acquaintance with the GitHub and its capabilities
- Done Task - Markdown & Git
- Making My CV Page
- Learning JS basics and solving codewars tasks-kata
- making Wildlife page

### Apprentice GOIT:

_7-days marathon - **August 2020** | Ukraine_

- learning basics of HTML5, CSS;
- making my own cv page;
- upload my [cv](https://pylkonatadreamresume.netlify.app/) to netlify.app
- listening lecture about Soft-skills

---

### _Example JavaScript code:_

```
function getCount(str) {
 var vowelsCount = 0;
  let arr = str.split('');

  let arrNew = arr.filter(a => {
    if (a === 'a' || a === 'e' || a === 'i' || a === 'o' || a === 'u') {
      vowelsCount++;
    }
  });

  return vowelsCount;
}

```

---

## Education:

_September **2002** - June **2007** | Ukraine_  
**Donetsk National Technical University**  
_**Management**_

---

## Courses/lectures:

1. **GoIT** 7-days-marathon: html, css basics knowledge, making cv page.
2. **Udemy course** - _“Web developper 2020”_ (in progress) - html, css, js basics knowledge.

---

## Languages:

- Ukrainian (native);
- Russian (native);
- English (pre-intermediate):
  - English courses (time to time);
  - language practice: WOW Classic EU server player;
  - Reading Harry Potter in original;

[1]: https://i.pinimg.com/736x/95/47/11/954711b0304528672ec1a2bb91a3cdd6--pokemon-images-anime-girls.jpg
